...
var snanmin = tryRequire( resolve( __dirname, './../lib/snanmin.native.js' ) );
var opts = {
	'skip': ( snanmin instanceof Error )
};
...
tape( 'the function throws an error if provided a first argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			snanmin( value, new Float32Array( 10 ), 1 );
		};
	}
});
...
tape( 'the function throws an error if provided a third argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			snanmin( 10, new Float32Array( 10 ), value );
		};
	}
});

tape( 'the function calculates the minimum value of a strided array', opts, function test( t ) {
	var x;
	var v;
